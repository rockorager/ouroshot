...
The build installs `share/ouro/mcp/apps/ouroshot.json`, generated by
`ouroshot-service --export-mcp-descriptor` from the same catalog as `tools/list`.
Install under an XDG data prefix to expose the service through `ouro-mcp`.

### Launch and socket activation

From a Wayland session, run `zig-out/bin/ouroshot-service` directly for native
clients. It binds `$XDG_RUNTIME_DIR/ouro/capture.mcp.sock` without replacing an
...
`ouroshot-capture.socket` uses `%t/ouro/capture.mcp.sock`, `Accept=no`, socket mode
...
EOF before final result commit or service shutdown cancels the worker and
dismisses selection. A parent-death signal prevents orphan workers after a crash.
`notifications/cancelled` with the active request ID also cancels without a
reply, leaving the connection available for further requests. Upgrade ourobridge
and the socket unit together: the former Varlink `capture.sock` is not supported.
...
Successful service operations require native user input, regardless of
...
Screenshot replies contain correctly percent-encoded absolute `file:///` URIs.
...
Native discovery example (requires Python 3):
...
# Real Screenshot selection/cancellation; also requires PyGObject.
dbus-run-session -- env OUROSHOT_PRIVATE_TEST_BUS=1 /usr/bin/python3 \
